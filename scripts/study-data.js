...
const STUDY_DATA = {
  ...
  cards: [
    ...
    { p:'p1', t:'t4', type:'tip', term:'Logs vs Metrics — Config Required', def:'Metrics are collected AUTOMATICALLY — no setup needed. Logs are NOT stored anywhere until you create a Diagnostic Setting and choose a destination. A top exam trap: "configure monitoring" almost always means Diagnostic Settings for logs.' },
    { p:'p1', t:'t4', type:'tip', term:'Budget Trap — Notifications Only', def:'Azure Cost Management budgets NEVER stop services when the threshold is hit. They only send email notifications at configured percentages (80%, 100%). Services keep running and costs accumulate. You must manually act to stop spending.' },
    ...
    { p:'p1', t:'t5', type:'tip', term:'Content Safety Scale — Even Numbers Only', def:'Content Safety severity values are ONLY 0, 2, 4, 6 — not a continuous 0–10 scale. 0=Safe, 2=Low, 4=Medium, 6=High. Exam questions may test whether you know only even values are used.' },
    { p:'p1', t:'t5', type:'tip', term:'Filters vs Blocklists vs Shields', def:'Content Filters = automatic severity scoring per harm category (Hate, Violence, Sexual, Self-Harm). Blocklists = exact term blocking. Prompt Shields = attack detection (jailbreak + indirect injection). Three distinct mechanisms, each tested separately.' },
    ...
    { p:'p3', t:'t1', type:'tip', term:'Thread vs Run — Know the Difference', def:'Thread = stateful container storing full conversation history across multiple executions. Run = one agent execution against a thread. Multiple runs can happen on one thread. "What persists conversation state?" → Thread. "What executes the agent?" → Run.' },
    { p:'p3', t:'t1', type:'tip', term:'Max Iterations — Agent Safety', def:'Always set a maximum step/iteration count on agent workflows. Without it, a stuck reasoning loop runs indefinitely at cost. This is the primary agent-specific guardrail tested on the exam — distinct from content filters.' },
    ...
    { p:'p4', t:'t2', type:'tip', term:'Speaker Verification vs Identification', def:"Speaker Verification = 'Is this person who they claim to be?' (1:1 match against enrolled profile). Speaker Identification = 'Which speaker is this?' (1:N match from group of enrolled profiles). Know which to use in scenario." },
    { p:'p4', t:'t2', type:'tip', term:'recognize_once() vs Continuous', def:"recognize_once() listens for one utterance then stops. For long audio (meetings, calls, hours of audio) use start_continuous_recognition() with event handlers for each recognized chunk. Exam tests this API distinction." },
